...
export class LiveChatClient {
  constructor(overrides={}) {
    ...
    this.requesterUserId = overrides.requesterUserId || config.lcRequesterUserId || this.accountId;
  ...
  async ensureRequesterInChat(chatId) {
    const id=String(chatId||'').trim();
    if(!id){ const er=new Error('LIVECHAT_CHAT_ID_REQUIRED'); er.status=400; throw er; }
    const userId=String(this.requesterUserId||this.accountId||'').trim();
    if(!userId){ const er=new Error('LIVECHAT_REQUESTER_USER_ID_REQUIRED'); er.status=500; throw er; }
    try{
